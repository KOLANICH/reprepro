...
retvalue dirs_create(const char *, int verbose);
/* create recursively all parent directories before the last '/' */
retvalue dirs_make_parent(const char *, int verbose);
/* create dirname and any '/'-separated part of it */
retvalue dirs_make_recursive(const char *, InterruptedFT interrupted, int verbose);
/* create directory and parents as needed, and save count to remove them later */
retvalue dir_create_needed(const char *, int *, InterruptedFT interrupted, int verbose);
void dir_remove_new(const char *, int, int verbose);
