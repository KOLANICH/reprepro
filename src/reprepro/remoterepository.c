...
static inline retvalue reuse_old_compressed_index(struct remote_distribution *rd, struct remote_index *ri, enum compression c, const char *oldfullfilename) {
	...
	r = uncompress_file(oldfullfilename, ri->cachefilename, c, interrupted, verbose);
	assert (r != RET_NOTHING);
	if (RET_WAS_ERROR(r))
		return r;
	if (ri->ofs[c_none] >= 0) {
...
static retvalue index_callback(enum queue_action action, void *privdata, UNUSED(void *privdata2), const char *uri, const char *gotfilename, const char *wantedfilename, /*@null@*/const struct checksums *gotchecksums, const char *methodname, int verbose) {
	struct remote_index *ri = privdata;
	struct remote_distribution *rd = ri->from;
	struct checksums *readchecksums = NULL;
	retvalue r;
	...
	if (ri->compression == c_none) {
		...
		r = uncompress_queue_file(gotfilename, ri->cachefilename,
				ri->compression,
				indexfile_unpacked, privdata, interrupted, verbose);
		if (RET_WAS_ERROR(r))
			return r;
		return RET_OK;
	}
...
static retvalue diff_got_callback(enum queue_action action, void *privdata, UNUSED(void *privdata2), UNUSED(const char *uri), const char *gotfilename, const char *wantedfilename, UNUSED(/*@null@*/const struct checksums *gotchecksums), UNUSED(const char *methodname), UNUSED(int verbose)) {
	struct remote_index *ri = privdata;
	retvalue r;
	...
	r = uncompress_queue_file(gotfilename, ri->patchfilename,
			c_gzip, diff_uncompressed, ri, interrupted, verbose);
	if (RET_WAS_ERROR(r))
		(void)unlink(gotfilename);
	return r;
}

static retvalue diff_callback(enum queue_action action, void *privdata, UNUSED(void *privdata2), const char *uri, const char *gotfilename, const char *wantedfilename, /*@null@*/const struct checksums *gotchecksums, const char *methodname, UNUSED(int verbose)) {
	struct remote_index *ri = privdata;
	struct remote_distribution *rd = ri->from;
	struct checksums *readchecksums = NULL;
	int ofs;
