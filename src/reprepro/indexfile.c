...
retvalue indexfile_open(struct indexfile **file_p, const char *filename, enum compression compression) {
	...
	r = uncompress_open(&f->f, filename, compression, interrupted);
	...
	if (FAILEDTOALLOC(f->buffer)) {
		uncompress_abort(f->f, interrupted);
		free(f->filename);
		free(f);
		return RET_ERROR_OOM;
	}
...
retvalue indexfile_close(struct indexfile *f) {
	...
	r = uncompress_close(f->f, interrupted);
...
"Error parsing %s line %d: Ridiculous long (>= 256K) control chunk!\n",
		...
		bytes_read = uncompress_read(f->f, d, f->size - f->ofs, interrupted);
		if (bytes_read < 0)
			return RET_ERROR;
