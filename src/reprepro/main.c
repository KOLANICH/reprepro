...
"Cannot uncompress format '%s'\nCheck __dumpuncompressors for more details.\n",
	...
	return uncompress_file(argv[2], argv[3], c, interrupted, verbose);
}
...
ACTION_N(n, n, y, extractcontrol) {
	...
	result = extractcontrol(&control, argv[1], interrupted);
...
"This most likely means the file was damaged or edited improperly\n",
	...
	while (sourceextraction_needs(extraction, &i)) {
		...
		r = sourceextraction_analyse(extraction, filename, interrupted);
		free(filename);
		if (RET_WAS_ERROR(r)) {
			result = r;
			break;
...
ACTION_D(y, n, y, update) {
	...
	result = dirs_make_recursive(global.listdir, interrupted, verbose);
	if (RET_WAS_ERROR(result)) {
		return result;
	}
...
ACTION_C(n, n, y, createsymlinks) {
	...
	r = dirs_make_recursive(global.distdir, interrupted, verbose);
	if (RET_WAS_ERROR(r))
		return r;
