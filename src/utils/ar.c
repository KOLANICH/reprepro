...
static ssize_t readwait(int fd, /*@out@*/void *buf, size_t count, InterruptedFT interrupted) {
	ssize_t totalread;
...
retvalue ar_open(/*@out@*/struct ar_archive **n, const char *filename, InterruptedFT interrupted) {
	...
	bytesread = readwait(ar->fd, buffer, sizeof(AR_MAGIC) - 1, interrupted);
...
retvalue ar_nextmember(struct ar_archive *ar, /*@out@*/char **filename, InterruptedFT interrupted) {
	ssize_t bytesread;
	char *p;
	off_t s;
...
"Error %d seeking to next member in ar file %s: %s\n",
	...
	bytesread = readwait(ar->fd, &ar->currentheader,
			sizeof(ar->currentheader), interrupted);
...
ssize_t ar_archivemember_read(struct archive *a, void *d, const void **p, InterruptedFT interrupted) {
	...
	*p = ar->readbuffer;
	bytesread = uncompress_read(ar->member, ar->readbuffer, BLOCKSIZE, interrupted);
	if (bytesread < 0) {
		const char *msg;
		int e;

		// TODO: why _fdclose instead of _abort?
		(void)uncompress_fdclose(ar->member, &e, &msg, interrupted);
		ar->member = NULL;
...
int ar_archivemember_open(struct archive *a, void *d, InterruptedFT interrupted) {
	struct ar_archive *ar = d;
	retvalue r;
	const char *msg;
	int e;
	...
	r = uncompress_fdopen(&ar->member, ar->fd, ar->member_size,
			ar->compression, &e, &msg, interrupted);
	if (RET_IS_OK(r))
		return ARCHIVE_OK;
	archive_set_error(a, e, "%s", msg);
	return ARCHIVE_FATAL;
...
int ar_archivemember_close(struct archive *a, void *d, InterruptedFT interrupted) {
	struct ar_archive *ar = d;
	retvalue r;
	const char *msg;
	int e;
	...
	r = uncompress_fdclose(ar->member, &e, &msg, interrupted);
	ar->member = NULL;
	if (RET_IS_OK(r))
		return ARCHIVE_OK;
	archive_set_error(a, e, "%s", msg);
