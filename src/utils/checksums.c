...
retvalue checksums_hardlink(const char *directory, const char *filekey, const char *sourcefilename, const struct checksums *checksums, int verbose) {
	retvalue r;
	int i, e;
	...
	if (i != 0 && (e == EACCES || e == ENOENT || e == ENOTDIR))  {
		errno = 0;
		(void)dirs_make_parent(fullfilename, verbose);
		i = link(sourcefilename, fullfilename);
		e = errno;
	}
	if (i != 0) {
...
retvalue checksums_linkorcopyfile(const char *destination, const char *source, struct checksums **checksums_p, int verbose) {
	int i;
	retvalue r;

	// TODO: is this needed? perhaps move this duty to the caller...
	r = dirs_make_parent(destination, verbose);
	if (RET_WAS_ERROR(r))
		return r;
