...
static retvalue parsediff(struct compressedfile *f, /*@null@*/char **section_p, /*@null@*/char **priority_p, bool *found_p, InterruptedFT interrupted) {
...
#define BUFSIZE 4096
	...
	inline char u_overlinegetchar(void) {
		...
		if (filled - used > 0) {
			...
			used = 0;
			bytes_read = uncompress_read(f, buffer, BUFSIZE, interrupted);
			if (bytes_read <= 0) {
				filled = 0;
				return '\0';
			}
...
#ifdef HAVE_LIBARCHIVE
static retvalue read_source_control_file(struct sourceextraction *e, struct archive *tar, struct archive_entry *entry, InterruptedFT interrupted) {
	// TODO: implement...
...
static ssize_t compressedfile_read(UNUSED(struct archive *a), void *d, const void **buffer_p, InterruptedFT interrupted) {
	struct compressedfile *f = d;
	// TODO malloc buffer instead
	static char mybuffer[4096];

	*buffer_p = mybuffer;
	return uncompress_read(f, mybuffer, 4096, interrupted);
}

static retvalue parse_tarfile(struct sourceextraction *e, const char *filename, enum compression c, /*@out@*/bool *found_p, InterruptedFT interrupted) {
	struct archive *tar;
	...
	r = uncompress_open(&file, filename, c, interrupted);
	if (!RET_IS_OK(r)) {
...
"Error trying to extract control information from %s:\n" "%s\n",
		...
		archive_read_free(tar);
		uncompress_abort(file, interrupted);
		return RET_ERROR;
	}
	while ((a=archive_read_next_header(tar, &entry)) == ARCHIVE_OK) {
		...
		if (iscontrol) {
			r = read_source_control_file(e, tar, entry, interrupted);
			archive_read_free(tar);
	...
	archive_read_free(tar);
	*found_p = false;
	return uncompress_close(file, interrupted);
}
#endif

/* full file name of requested files ready to analyse */
retvalue sourceextraction_analyse(struct sourceextraction *e, const char *fullfilename, InterruptedFT interrupted) {
	retvalue r;
	bool found;
	...
	if (e->difffile >= 0) {
		...
		r = uncompress_open(&f, fullfilename, e->diffcompression, interrupted);
		if (!RET_IS_OK(r)) {
			e->failed = true;
			/* being unable to read a file is no hard error... */
			return RET_NOTHING;
		}
		r = parsediff(f, e->section_p, e->priority_p, &found, interrupted);
		if (RET_IS_OK(r))  {
			if (!found)
...
#ifdef HAVE_LIBARCHIVE
	r = parse_tarfile(e, fullfilename, e->tarcompression, &found, interrupted);
	if (!RET_IS_OK(r))
		e->failed = true;
	else if (found)
		/* do not look in the tar, we found debian/control */
