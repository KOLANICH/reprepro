...
static retvalue checksigs(const char *filename, struct signatures **signatures_p, bool *broken, int verbose) {
...
#ifdef HAVE_LIBGPGME
static retvalue extract_signed_data(const char *buffer, size_t bufferlen, const char *filenametoshow, char **chunkread, /*@null@*/ /*@out@*/struct signatures **signatures_p, bool *brokensignature, int verbose) {
	char *chunk;
	gpg_error_t err;
...
"Data seems not to be signed trying to use directly....\n");
		...
		if (signatures_p != NULL || brokensignature != NULL) {
			r = checksigs(filenametoshow,
				...
				(brokensignature!=NULL)?&foundbroken:NULL, verbose);
			if (RET_WAS_ERROR(r)) {
				gpgme_data_release(dh_gpg);
				gpgme_data_release(dh);
				return r;
...
retvalue signature_readsignedchunk(const char *filename, const char *filenametoshow, char **chunkread, /*@null@*/ /*@out@*/struct signatures **signatures_p, bool *brokensignature, int verbose) {
	char *chunk;
...
"Data seems not to be signed trying to use directly...\n");
	...
	r = extract_signed_data(chunk, chunklen, filenametoshow, chunkread,
			signatures_p, brokensignature, verbose);
	if (r != RET_NOTHING) {
		free(chunk);
		return r;
	}
