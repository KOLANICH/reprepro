...
static retvalue uncompress_start_queued(int verbose) {
...
static inline retvalue builtin_uncompress(const char *compressed, const char *destination, enum compression compression, InterruptedFT interrupted);

/* we got an pid, check if it is a uncompressor we care for */
retvalue uncompress_checkpid(pid_t pid, int status, InterruptedFT interrupted, int verbose) {
...
"'%s' < %s > %s exited with errorcode %d!\n",
	...
	if (error && uncompression_builtin(t->compression)) {
		...
		r = builtin_uncompress(t->compressedfilename,
				t->uncompressedfilename, t->compression, interrupted);
		if (RET_WAS_ERROR(r)) {
			(void)unlink(t->uncompressedfilename);
		} else if (RET_IS_OK(r)) {
			error = false;
	...
	*t_p = t->next;
	...
	r2 = uncompress_start_queued(verbose);
	RET_ENDUPDATE(r, r2);
	return r;
}

bool uncompress_running(int verbose) {
	uncompress_start_queued(verbose);
	return tasks != NULL;
}
...
static inline retvalue builtin_uncompress(const char *compressed, const char *destination, enum compression compression, InterruptedFT interrupted) {
	...
	r = uncompress_open(&f, compressed, compression, interrupted);
	if (!RET_IS_OK(r))
		return r;
	...
	if (destfd < 0) {
		...
		uncompress_abort(f, interrupted);
		return RET_ERRNO(e);
	}
	do {
		bytes_read = uncompress_read(f, buffer, 4096, interrupted);
		if (bytes_read <= 0)
			break;
...
"Error %d writing to '%s': %s\n",
	...
	r = uncompress_close(f, interrupted);
	if (RET_WAS_ERROR(r)) {
		(void)close(destfd);
		return r;
	}
...
static retvalue uncompress_queue_external(enum compression compression, const char *compressed, const char *uncompressed, /*@null@*/finishaction *action, /*@null@*/void *privdata, int verbose) {
	struct uncompress_task *t, **t_p;
	retvalue r;
	...
	t->privdata = privdata;
	*t_p = t;
	r = uncompress_start_queued(verbose);
	if (r == RET_NOTHING)
		r = RET_ERROR_INTERNAL;
	return r;
}

retvalue uncompress_queue_file(const char *compressed, const char *destination, enum compression compression, finishaction *action, void *privdata, InterruptedFT interrupted, int verbose) {
	retvalue r;

	(void)unlink(destination);
	if (extern_uncompressors[compression] != NULL) {
		r = uncompress_queue_external(compression, compressed,
				destination, action, privdata, verbose);
		if (r != RET_NOTHING) {
			return r;
		}
	...
	r = builtin_uncompress(compressed, destination, compression, interrupted);
	if (RET_WAS_ERROR(r)) {
		(void)unlink(destination);
		return r;
	}
...
retvalue uncompress_file(const char *compressed, const char *destination, enum compression compression, InterruptedFT interrupted, int verbose) {
	retvalue r;
	...
	if (uncompression_builtin(compression)) {
		...
		r = uncompress_queue_external(compression,
				compressed, destination, NULL, NULL, verbose);
		if (r == RET_NOTHING)
			r = RET_ERROR;
		if (RET_IS_OK(r)) {
...
"Error %d waiting for uncompression child: %s\n",
					...
					r = uncompress_checkpid(pid, status, interrupted, verbose);
			} while (r == RET_NOTHING);
		}
	} else {
...
static retvalue uncompression_read_internal_buffer(struct compressedfile *f, InterruptedFT interrupted) {
	size_t len;
	ssize_t r;
...
static retvalue start_builtin(struct compressedfile *f, int *errno_p, const char **msg_p, InterruptedFT interrupted) {
	retvalue r;

	assert (f->compression != c_none);
	assert (uncompression_builtin(f->compression));

	r = uncompression_read_internal_buffer(f, interrupted);
	if (RET_WAS_ERROR(r)) {
...
retvalue uncompress_open(/*@out@*/struct compressedfile **file_p, const char *filename, enum compression compression, InterruptedFT interrupted) {
	struct compressedfile *f;
	int fd, e;
	retvalue r;
	const char *msg;
	...
	if (compression == c_none || uncompression_builtin(compression)) {
		...
		if (f->compression != c_none) {
			r = start_builtin(f, &e, &msg, interrupted);
			if (RET_WAS_ERROR(r)) {
				(void)close(f->fd);
				if (e != -EINVAL && e != 0)
					fprintf(stderr,
...
retvalue uncompress_fdopen(struct compressedfile **file_p, int fd, off_t len, enum compression compression, int *errno_p, const char **msg_p, InterruptedFT interrupted) {
	struct compressedfile *f;
	retvalue r;
	...
	if (compression == c_none) {
		...
		f->fd = fd;
		f->infd = -1;
		r = start_builtin(f, errno_p, msg_p, interrupted);
		if (RET_WAS_ERROR(r)) {
			free(f);
			return r;
		}
...
static inline int restart_gz_if_needed(struct compressedfile *f, InterruptedFT interrupted) {
	retvalue r;
	int ret;
...
static inline int read_gz(struct compressedfile *f, void *buffer, int size, InterruptedFT interrupted) {
	int ret;
	...
	do {
		...
		if (ret == Z_STREAM_END) {
			...
			if (!restart_gz_if_needed(f, interrupted))
				return -1;
...
#ifdef HAVE_LIBBZ2
static inline int restart_bz2_if_needed(struct compressedfile *f, InterruptedFT interrupted) {
	retvalue r;
	int ret;
...
static inline int read_bz2(struct compressedfile *f, void *buffer, int size, InterruptedFT interrupted) {
	int ret;
	retvalue r;
	bool eoi;
	...
	do {
		...
		if (ret == BZ_STREAM_END) {
			...
			if (!restart_bz2_if_needed(f, interrupted))
				return -1;
			if (gotdata > 0 || f->uncompress.hadeos)
				return gotdata;
...
#ifdef HAVE_LIBLZMA
static inline int read_lzma(struct compressedfile *f, void *buffer, int size, InterruptedFT interrupted) {
	int ret;
	retvalue r;
	bool eoi;
...
int uncompress_read(struct compressedfile *file, void *buffer, int size, InterruptedFT interrupted) {
	ssize_t s;
	...
	switch (file->compression) {
		case c_none:
			...
			return s;
		case c_gzip:
			return read_gz(file, buffer, size, interrupted);
#ifdef HAVE_LIBBZ2
		case c_bzip2:
			return read_bz2(file, buffer, size, interrupted);
#endif
#ifdef HAVE_LIBLZMA
		case c_xz:
		case c_lzma:
			return read_lzma(file, buffer, size, interrupted);
#endif
		default:
			assert (false);
			return -1;
...
static retvalue uncompress_commonclose(struct compressedfile *file, int *errno_p, const char **msg_p, InterruptedFT interrupted) {
	retvalue result;
	int ret;
	int e;
	pid_t pid;
...
void uncompress_abort(struct compressedfile *file, InterruptedFT interrupted) {
	pid_t pid;
	int e, status;
...
retvalue uncompress_fdclose(struct compressedfile *file, int *errno_p, const char **msg_p, InterruptedFT interrupted) {
	retvalue r;

	assert(file->closefd == false);
	r = uncompress_commonclose(file, errno_p, msg_p, interrupted);
	free(file);
	return r;
}

retvalue uncompress_close(struct compressedfile *file, InterruptedFT interrupted) {
	const char *msg;
	retvalue r;
	int e;
	...
	r = uncompress_commonclose(file, &e, &msg, interrupted);
	if (RET_IS_OK(r)) {
