...
retvalue dirs_create(const char *dirname, int verbose) {
	int ret, e;
...
retvalue dirs_make_parent(const char *filename, int verbose) {
	const char *p;
	char *h;
	int i;
	retvalue r;
...
retvalue dirs_make_recursive(const char *directory, InterruptedFT interrupted, int verbose) {
	retvalue r, result;

	if (interrupted()) {
		return RET_ERROR_INTERRUPTED;
	}
	r = dirs_make_parent(directory, verbose);
	result = dirs_create(directory, verbose);
	RET_UPDATE(result, r);
	return result;
}

/* create directory and return the number of created directoried */
retvalue dir_create_needed(const char *directory, int *createddepth, InterruptedFT interrupted, int verbose) {
	retvalue r;
	int ret;
...
void dir_remove_new(const char *directory, int created, int verbose) {
	size_t len = strlen(directory);
	char *this;
	int ret;
