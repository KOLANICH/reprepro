...
static retvalue read_control_file(char **control, const char *debfile, struct archive *tar, struct archive_entry *entry, InterruptedFT interrupted) {
	int64_t size;
	char *buffer, *n;
...
static retvalue read_control_tar(char **control, const char *debfile, struct ar_archive *ar, struct archive *tar, InterruptedFT interrupted) {
...
"Error skipping %s within data.tar.gz from %s: %d=%s\n",
			...
			r = read_control_file(control, debfile, tar, entry, interrupted);
			if (r != RET_NOTHING)
				return r;
		}
	}
...
retvalue extractcontrol(char **control, const char *debfile, InterruptedFT interrupted) {
	struct ar_archive *ar;
	retvalue r;
	bool hadcandidate = false;

	r = ar_open(&ar, debfile, interrupted);
	if (RET_WAS_ERROR(r))
		return r;
	...
	do {
		...
		r = ar_nextmember(ar, &filename, interrupted);
		if (RET_IS_OK(r)) {
			...
			if (uncompression_supported(c)) {
				...
				r = read_control_tar(control, debfile, ar, tar, interrupted);
